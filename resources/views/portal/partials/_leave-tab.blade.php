...
<div x-data="leaveMgmt({
        rows: {{ Js::from($leaveRowsArr) }},
        departments: {{ Js::from($leaveDepartments->map(fn($d) => ['id'=>$d->id,'name'=>$d->name])->values()) }},
        policies: {{ Js::from($leavePolicies->map(fn($p) => ['id'=>$p->id,'name'=>$p->name])->values()) }},
        year: {{ $leaveYear }}
     })">

    {{-- Sub-header with year picker --}}
    <div class="flex items-center justify-between mb-4">
        <p class="text-sm text-gray-500">ภาพรวมสิทธิวันลาของพนักงานทุกคน • ปี {{ $leaveYear + 543 }}</p>
        <div class="flex items-center gap-2">
            <form method="GET" class="flex items-center gap-2">
                <input type="hidden" name="tab" value="leave">
                <label class="text-xs text-gray-500">ปี:</label>
                <select name="leave_year" onchange="this.form.submit()" class="px-3 py-1.5 border rounded-lg text-sm">
                    @for($y = now()->year - 2; $y <= now()->year + 1; $y++)
                        <option value="{{ $y }}" @selected($y === $leaveYear)>{{ $y + 543 }}</option>
                    @endfor
                </select>
            </form>
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <div class="text-2xl font-extrabold text-gray-800 mt-1">{{ $leaveStats['total_employees'] }}</div>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <div class="text-[10px] font-bold text-gray-500 uppercase">นโยบายที่ใช้</div>
            <div class="text-2xl font-extrabold text-indigo-700 mt-1">{{ $leaveStats['total_policies'] }}</div>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <div class="text-[10px] font-bold text-gray-500 uppercase">ยอดยกข้ามปี</div>
            <div class="text-2xl font-extrabold text-emerald-700 mt-1">{{ rtrim(rtrim(number_format($leaveStats['total_carryover_days'], 1), '0'), '.') }}</div>
            <div class="text-[10px] text-gray-400">วัน</div>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <div class="text-[10px] font-bold text-gray-500 uppercase">แลกเป็นเงิน</div>
            <div class="text-2xl font-extrabold text-rose-700 mt-1">{{ rtrim(rtrim(number_format($leaveStats['total_encash_days'], 1), '0'), '.') }}</div>
            <div class="text-[10px] text-gray-400">วัน</div>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <div class="text-[10px] font-bold text-gray-500 uppercase">มูลค่าแลก</div>
            <div class="text-xl font-extrabold text-amber-600 mt-1">฿{{ number_format($leaveStats['total_encash_amount'], 0) }}</div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 mb-3 space-y-3">
        ...
        <div class="flex flex-wrap gap-2 items-center">
            ...
            @php
                $chips = [
                    'all' => ['label' => 'ทั้งหมด', 'color' => 'gray', 'count' => $leaveStats['total_employees']],
                    'low_vacation' => ['label' => '🔴 พักร้อนเหลือน้อย (<5)', 'color' => 'rose', 'count' => $leaveStats['low_vacation_count']],
                    'no_policy' => ['label' => '⚠️ ไม่มีนโยบาย', 'color' => 'amber', 'count' => $leaveStats['no_policy_count']],
                    'probation' => ['label' => '🆕 ทดลองงาน', 'color' => 'sky', 'count' => $leaveStats['probation_count']],
                    'expiring' => ['label' => '⏰ ยกยอดใกล้หมดอายุ', 'color' => 'purple', 'count' => $leaveStats['expiring_count']],
                    'has_unused_vacation' => ['label' => '🏖️ พักร้อนเหลือ + ยกยอดได้', 'color' => 'teal', 'count' => $leaveUnusedVacationCount],
                ];
            @endphp
            ...
            <form method="POST" action="{{ route('leave-management.batch-encash') }}" class="p-6">
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <input type="number" name="year" required value="{{ $leaveYear }}" class="w-full px-3 py-2 border rounded-lg text-sm">
                    </div>
                ...
                <template x-if="!historyLoading && historyData">
                    <div class="space-y-5">
                        ...
                        <div>
                            ...
                            <template x-if="!historyData.audit_logs || historyData.audit_logs.length === 0">
                                ...
                                <template x-for="a in historyData.audit_logs" :key="a.id">
                                    <div class="p-2.5 bg-slate-50 border border-slate-100 rounded-lg text-xs">
                                        ...
                                        <div class="text-[10px] text-slate-500">
                                            ...
                                            <template x-if="a.reason"><span x-text="' · เหตุผล: ' + a.reason"></span></template>
                                        </div>
                                    </div>
                                </template>
                            </div>
            ...
            <form method="POST" action="{{ route('leave-management.batch-carryover') }}" class="space-y-3">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="number" name="source_year" required value="{{ $leaveYear - 1 }}"
                               class="w-full px-3 py-2 border rounded-lg text-sm">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">ปีปลายทาง</label>
                        <input type="number" name="target_year" required value="{{ $leaveYear }}"
                               class="w-full px-3 py-2 border rounded-lg text-sm">
                    </div>
                </div>
                <label class="flex items-center gap-2 p-2 bg-amber-50 border border-amber-100 rounded-lg">
