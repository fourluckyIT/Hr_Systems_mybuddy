...
@php
    ...
    $tabs = [
        ...
        ['key' => 'holidays',   'label' => 'วันหยุดบริษัท',   'icon' => '📅'],
...
<div x-data="{
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div x-show="tab === 'holidays'" x-cloak
             x-data="{
                 filter: 'all',
                 typeId: '{{ $holidayTypes->firstWhere('code', 'company')?->id }}',
                 typeMap: {{ $holidayTypes->mapWithKeys(fn($t) => [$t->id => ['color' => $t->default_color, 'icon' => $t->icon, 'name' => $t->name]])->toJson() }},
                 get currentType() { return this.typeMap[this.typeId] ?? { color: 'sky', icon: '🏢', name: '' } },
             }">

            {{-- Stats: usage of holidays for the current year --}}
            <div class="mb-5 grid grid-cols-2 md:grid-cols-4 gap-2">
                <div class="p-3 rounded-xl bg-gray-50 border border-gray-100">
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider">ปี {{ $holidayStats['year'] }}</p>
                    <p class="mt-1 flex items-baseline gap-1">
                        <span class="text-2xl font-black text-gray-800">{{ $holidayStats['total'] }}</span>
                        <span class="text-[11px] text-gray-400">วันทั้งหมด</span>
                    </p>
                </div>
                <div class="p-3 rounded-xl bg-gray-50 border border-gray-100">
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider">ผ่านไปแล้ว</p>
                    <p class="mt-1 flex items-baseline gap-1">
                        <span class="text-2xl font-black text-gray-500">{{ $holidayStats['passed'] }}</span>
                        <span class="text-[11px] text-gray-400">/ {{ $holidayStats['total'] ?: '—' }}</span>
                    </p>
                </div>
                <div class="p-3 rounded-xl bg-emerald-50 border border-emerald-100">
                    <p class="text-[10px] font-bold text-emerald-600 uppercase tracking-wider">เหลืออีก</p>
                    <p class="mt-1 flex items-baseline gap-1">
                        <span class="text-2xl font-black text-emerald-700">{{ $holidayStats['upcoming'] }}</span>
                        <span class="text-[11px] text-emerald-500">วัน</span>
                    </p>
                </div>
                <div class="p-3 rounded-xl bg-purple-50 border border-purple-100">
                    <p class="text-[10px] font-bold text-purple-600 uppercase tracking-wider">วันนี้</p>
                    @if($holidayStats['today'] > 0)
                        <p class="mt-1 text-sm font-bold text-purple-700">🎉 เป็นวันหยุด!</p>
                    @else
                        <p class="mt-1 text-sm text-purple-400">ไม่ใช่วันหยุด</p>
                    @endif
                </div>
            </div>

            {{-- Progress bar (passed / total) --}}
            @if($holidayStats['total'] > 0)
                @php $pct = round(($holidayStats['passed'] / $holidayStats['total']) * 100); @endphp
                <div class="mb-5">
                    <div class="flex items-center justify-between text-[10px] text-gray-400 mb-1">
                        <span>ความคืบหน้าปี {{ $holidayStats['year'] }}</span>
                        <span class="font-bold">{{ $pct }}%</span>
                    </div>
                    <div class="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                        <div class="h-full bg-gradient-to-r from-emerald-400 to-purple-400 transition-all" style="width: {{ $pct }}%"></div>
                    </div>
                </div>
            @endif
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
                <div class="flex items-center gap-1.5 text-[11px] flex-wrap">
                    <span class="text-gray-400 mr-1">สี:</span>
                    @foreach($holidayTypes as $t)
                        <button type="button" @click="filter = (filter === '{{ $t->id }}' ? 'all' : '{{ $t->id }}')"
                                :class="filter === '{{ $t->id }}' ? 'ring-2 ring-{{ $t->default_color }}-400 ring-offset-1' : ''"
                                class="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-{{ $t->default_color }}-100 text-{{ $t->default_color }}-700 font-semibold hover:bg-{{ $t->default_color }}-200 transition">
                            <span>{{ $t->icon }}</span>
                            <span>{{ $t->name }}</span>
                        </button>
                    @endforeach
                    <button type="button" @click="filter = 'all'" x-show="filter !== 'all'"
                            class="text-gray-400 hover:text-gray-600 underline ml-1">ล้างตัวกรอง</button>
                </div>

                <form action="{{ route('settings.holidays.load-legal') }}" method="POST" class="flex items-center gap-2">
                    @csrf
                    <input type="number" name="year" value="{{ now()->year }}" min="2020" max="2099"
                           class="w-20 px-2 py-1.5 border border-purple-200 rounded-lg text-xs font-bold focus:ring-1 focus:ring-purple-500 bg-purple-50">
                    <button type="submit" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-purple-600 text-white rounded-lg text-xs font-bold hover:bg-purple-700 transition">
                        <span>📥</span>
            ...
            {{-- Compact add form --}}
            <form action="{{ route('settings.holidays.add') }}" method="POST"
                  class="mb-5 p-3 rounded-xl border border-gray-200 bg-gradient-to-br"
                  :class="`from-${currentType.color}-50 to-white`">
                @csrf
                <div class="flex flex-col md:flex-row gap-2 items-stretch md:items-end">
                    <div class="flex-grow">
                        <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">ชื่อวันหยุด</label>
                        <input type="text" name="name" required placeholder="เช่น วันหยุดประจำปี, วันสัมมนา..."
                               class="w-full px-3 py-2 border rounded-lg text-sm bg-white focus:ring-2"
                               :class="`focus:ring-${currentType.color}-400`">
                    </div>
                    <div class="md:w-44">
                        <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">วันที่</label>
                    ...
                    <div class="md:w-56">
                        ...
                        <select name="holiday_type_id" x-model="typeId"
                                class="w-full px-3 py-2 border rounded-lg text-sm bg-white focus:ring-2"
                                :class="`focus:ring-${currentType.color}-400`">
                            @foreach($holidayTypes as $t)
                    ...
                    <button type="submit" class="px-5 py-2 text-white rounded-lg font-bold text-sm transition shrink-0 shadow-sm"
                            :class="`bg-${currentType.color}-600 hover:bg-${currentType.color}-700`">
                        + เพิ่ม
                    </button>
                </div>
                <p class="mt-2 text-[10px] text-gray-400">
                    💡 สีจะแสดงตามประเภทอัตโนมัติ — กำหนดสีของแต่ละประเภทได้ที่
                    <a href="{{ route('settings.master-data') }}" class="text-purple-600 hover:underline">Master Data → ประเภทวันหยุด</a>
                </p>
            </form>

            {{-- Holiday list grouped by year --}}
            @php
                $byYear = $holidays->groupBy(fn($h) => $h->holiday_date->format('Y'))->sortKeys();
            @endphp

            @if($holidays->isEmpty())
                <div class="py-12 text-center bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
                    <div class="text-4xl mb-2">📅</div>
                    <p class="text-sm font-bold text-gray-500">ยังไม่มีวันหยุด</p>
                    <p class="text-xs text-gray-400 mt-1">ลองกด "ดึงวันหยุดราชการ" หรือเพิ่มวันหยุดบริษัทเอง</p>
                </div>
            @else
                @foreach($byYear as $year => $yearHolidays)
                    <div class="mb-5">
                        <div class="flex items-center gap-2 mb-2 sticky top-0 bg-white py-1 z-10">
                            <h4 class="text-xs font-bold text-gray-500 uppercase tracking-wider">{{ $year }}</h4>
                            <span class="text-[10px] text-gray-400">· {{ $yearHolidays->count() }} วัน</span>
                            <div class="flex-grow border-b border-gray-100"></div>
                        </div>

                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                            @foreach($yearHolidays as $holiday)
                                @php $c = $holiday->effective_color; $tid = $holiday->holiday_type_id; @endphp
                                <div x-show="filter === 'all' || filter === '{{ $tid }}'"
                                     class="group flex items-center gap-3 p-2.5 bg-white border border-gray-100 rounded-xl hover:border-{{ $c }}-300 hover:shadow-sm transition-all">
                                    {{-- Date block --}}
                                    <div class="w-12 h-12 bg-{{ $c }}-100 text-{{ $c }}-700 rounded-lg flex flex-col items-center justify-center flex-shrink-0">
                                        <span class="text-[8px] font-bold uppercase leading-none">{{ $holiday->holiday_date->locale('th')->isoFormat('MMM') }}</span>
                                        <span class="text-base font-black leading-tight">{{ $holiday->holiday_date->format('d') }}</span>
                                    </div>
                                    {{-- Name + type pill --}}
                                    <div class="flex-grow min-w-0">
                                        <p class="text-[13px] font-bold text-gray-800 truncate">{{ $holiday->name }}</p>
                                        <div class="flex items-center gap-1.5 mt-0.5">
                                            <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-bold bg-{{ $c }}-50 text-{{ $c }}-700">
                                                <span>{{ $holiday->effective_icon }}</span>
                                                <span>{{ $holiday->holidayType?->name ?? 'วันหยุด' }}</span>
                                            </span>
                                            <span class="text-[10px] text-gray-400">{{ $holiday->holiday_date->locale('th')->isoFormat('ddd') }}</span>
                                        </div>
                                    </div>
                                    {{-- Delete --}}
                                    <form action="{{ route('settings.holidays.delete', $holiday->id) }}" method="POST" onsubmit="return confirm('ลบ \'{{ $holiday->name }}\'?')" class="flex-shrink-0">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="p-1.5 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg opacity-0 group-hover:opacity-100 transition">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6"/></svg>
                                        </button>
                                    </form>
                                </div>
