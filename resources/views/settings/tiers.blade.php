...
@section('title', 'Global Tier Settings')
...
<div class="max-w-6xl mx-auto space-y-6" x-data="{
    ...
    {{-- Flash Messages --}}
    @if(session('success'))
    <div class="p-4 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-xl text-sm font-semibold">✓ {{ session('success') }}</div>
    @endif
    @if($errors->any())
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Performance Tiers</h1>
            <p class="text-sm text-gray-500 mt-1">จัดการระดับผลงาน (Global Tier) และเกณฑ์การประเมินอัตโนมัติ</p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('settings.bonus.index') }}" class="px-4 py-2 bg-gray-100 text-gray-600 rounded-xl text-sm font-semibold hover:bg-gray-200 transition-colors">ย้อนกลับ</a>
